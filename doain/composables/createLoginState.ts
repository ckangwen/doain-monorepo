...
import { httpClient, setToken, useUserStore } from "../core/index";
...
export const createLoginState = (options: CreateLoginStateOptions) => {
  ...
  return function useLogin({ afterLogin, beforeLogin }: UseLoginOptions) {
    const getTokenAfterLogin: DoainClientConfig["fetch"]["getTokenAfterLogin"] =
      userDoainConfig?.fetch?.getTokenAfterLogin;
    const loginApi = userDoainConfig?.fetch?.login;
    ...
    const onSubmitSuc = async (data: FormValue, message: string) => {
      ...
      if (userDoainConfig?.router?.homeRoute) {
        router?.push(userDoainConfig.router.homeRoute);
      }
    };
